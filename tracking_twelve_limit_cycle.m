function [] = tracking_twelve_limit_cycle(L,T,a,b,g,e)
...
%% Examples
% tracking_twelve_limit_cycle(100,5001,10,10,10,100) w/ slow circ
% tracking_twelve_limit_cycle(100,5001,5,5,50,100) w/ stationary
...
% Keep track of the limit-cycle values (polar angle and velocity angle) for
% a SINGLE chosen drone.
lim_cycle_A = zeros(T,2);
choice = 1;
y_chosen_V = y_unit_A(choice,:);
v_chosen_V = v_unit_A(choice,:);
theta = atan(y_chosen_V(1)/y_chosen_V(2)) + pi;
theta = mod(theta,2*pi);
psi = atan(y_chosen_V(1)/y_chosen_V(2));
lim_cycle_A(1,:) = [theta,psi];
...
%     dash_sum_V = dash_sum_V./(dash_factor_V).^zeta;
    ...
    % Update the limit cycle values
    y_chosen_V = y_unit_A(choice,:);
    v_chosen_V = v_unit_A(choice,:);
    theta = atan(y_chosen_V(1)/y_chosen_V(2));
    theta = mod(theta,2*pi);
    psi = atan(v_chosen_V(1)/v_chosen_V(2));
    lim_cycle_A(t,:) = [theta,psi];
...
% figure()
% plot(lim_cycle_A(:,1),lim_cycle_A(:,2))
