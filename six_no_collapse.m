function [] = six_no_collapse(L,T,a,b,g,m)
...
%% Examples
% six_no_collapse(200,5001,10,10,0.2,0)
% six_no_collapse(200,10001,30,30,0.2,0)
...
dro_pos_A = repmat(tar_pos_V,6,1) + 5*randn(6,2);
centroid_V = sum(dro_pos_A,1)/6;
...
if 0
    ...
    for i = 1:5
        quiver(repmat(dro_pos_A(i,1),14,1),repmat(dro_pos_A(i,2),14,1),...
...
dro_traj_A = zeros(T,12);
dro_direc_A = zeros(T,12);
dro_traj_A(1,:) = reshape(dro_pos_A',1,12);
dro_direc_A(1,:) = reshape(dro_vel_A',1,12);
...
h3 = animatedline('Color','r','MaximumNumPoints',50);
h4 = animatedline('Color','m','MaximumNumPoints',50);
h5 = animatedline('Color','k','MaximumNumPoints',50);
h6 = animatedline('Color','b','MaximumNumPoints',50,'LineStyle','-.');
h7 = animatedline('Color','r','MaximumNumPoints',50,'LineStyle','-.');
[R_calc] = radius_calc(a,b,g,5)
sqax = R_calc + 10;
...
for t = 2:T
    ...
    if mod(t,200) == 0
        ...
        t_count = t
        [R_est] = radius_est(dro_pos_A,6)
    end
    
    % Reshape the arrays.
    dro_pos_A = reshape(dro_pos_prev_V,2,6)';
    dro_vel_A = reshape(dro_vel_prev_V,2,6)';
    ...
    y_repulsion_V = reshape(y_repulsion_A',1,12);
...
%     end
    ...
    dro_direc_A(t,:) = dro_vel_prev_V + ...
        (...
        alpha*reshape(y_unit_A',1,12) - ...
        beta*repmat(v_repulsion_V,1,6) + ...
        beta*reshape(v_unit_A',1,12) - ...
        dro_vel_prev_V - ...
        gamma*gamma_V - ...
        mu*y_repulsion_V)*dt;
