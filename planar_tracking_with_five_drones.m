function [] = planar_tracking_with_five_drones()
...
%% Examples
% [] = planar_tracking_with_five_drones()
...
drone_pos_array = repmat(target_pos_vec,5,1) + randn(5,2);

% Fixed pos. initialisation.
% drone_pos_array = [-2 -1 0 1 2; 
%                    -2 -2 -2 -2 -2]'; 

% Stationary initialisation.
drone_vel_array = zeros(5,2);  

% Random vel. initialisation.
% drone_vel_array = randn(5,2);                
...
est_pos_array = repmat(target_pos_vec,5,1) + 0.1*randn(5,2);
...
[r_unit_direction_array] = direction_finder_five(drone_pos_array);
[v_unit_orientation_array] = orientation_finder_five(drone_vel_array);
[y_unit_target_dir_array] = target_finder_five(drone_pos_array,...
                                          target_pos_vec);
...
drone_trajectory_array = zeros(T,10);
drone_dir_array = zeros(T,10);
drone_trajectory_array(1,:) = reshape(drone_pos_array',1,10);
drone_dir_array(1,:) = reshape(drone_vel_array',1,10);
...
g6 = animatedline('Color','c','MaximumNumPoints',1,'Marker','o');
...
if 1
    ...
    h6 = animatedline('Color','c','MaximumNumPoints',100);
else
...
for t = 2:T
    ...
    % Reshape the arrays.
    drone_pos_array = reshape(drone_pos_previous,2,5)';
    drone_vel_array = reshape(drone_vel_previous,2,5)';
    ...
    drone_dir_array(t,:) = drone_vel_previous + ...
        (alpha*reshape(y_unit_target_dir_array',1,10) - ...
        beta*repmat(v_sum,1,5) + ...
        beta*reshape(v_unit_orientation_array',1,10) - ...
        drone_vel_previous)*dt;
    drone_vel_previous = drone_dir_array(t,:);
    
    % Add new points to the animated lines.
    ...
    addpoints(h6,drone_trajectory_array(t,9),drone_trajectory_array(t,10));
    ...
    addpoints(g6,drone_trajectory_array(t,9),drone_trajectory_array(t,10));
    drawnow;
